...
function Start-LocalSupabase {
    ...
    # Recover from a partial/exited container state (e.g. after a machine restart or Docker crash).
    Write-Host "Supabase start failed — attempting recovery (stop then start)..." -ForegroundColor Yellow
    & $SupabaseCli stop --project-id $projectId
    & $SupabaseCli start

    if ($LASTEXITCODE -ne 0) {
        throw "Supabase failed to start. Check whether ports 54321-54329 are already in use."
    }
}
