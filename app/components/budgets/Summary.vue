  ...
  <div class="flex flex-col gap-4">
    ...
    <template v-if="store.loading">
      <div class="grid grid-cols-2 gap-3">
        <USkeleton v-for="n in 4" :key="n" class="h-14 rounded-lg opacity-40" />
      </div>
      <div class="space-y-2">
        <USkeleton v-for="n in 5" :key="n" class="h-10 rounded-lg opacity-40" />
      </div>
    </template>
    ...
    <template v-else>
      ...
      <div>
        <ul class="grid grid-cols-2 gap-2">
          <li
            v-for="(budget, idx) in allBudgets"
            :key="budget.id"
            class="flex items-center gap-2 rounded-lg px-3 py-2 cursor-pointer transition-colors"
            :class="highlightedIndex === idx ? 'bg-default ring-2 ring-inset' : 'bg-elevated'"
            :style="highlightedIndex === idx && budget.color ? { '--tw-ring-color': budget.color } : {}"
          ...
          <li v-if="allBudgets.length === 0" class="text-sm text-muted text-center py-2">No budgets set.</li>
        </ul>
      </div>
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="rounded-lg bg-elevated p-2">
          <p class="text-xs text-gray-400 mb-0.5">Total Income</p>
          <p class="text-lg font-bold">${{ totalIncome.toLocaleString('en-US', { maximumFractionDigits: 0 }) }}</p>
        </div>
        <div class="rounded-lg bg-elevated p-2">
          <p class="text-xs text-gray-400 mb-0.5">% Income Budgeted</p>
          <p class="text-lg font-bold" :class="percentIncomeBudgeted > 100 ? 'text-red-400' : 'text-primary'">
            {{ percentIncomeBudgeted.toFixed(0) }}%
          </p>
        </div>
        <div class="rounded-lg bg-elevated p-2">
          <p class="text-xs text-gray-400 mb-0.5">Total Budgeted</p>
          <p class="text-lg font-bold">${{ totalBudgeted.toLocaleString('en-US', { maximumFractionDigits: 0 }) }}</p>
        </div>
        <div class="rounded-lg bg-elevated p-2">
          <p class="text-xs text-gray-400 mb-0.5">Remaining Funds</p>
          <p class="text-lg font-bold" :class="remainingFunds < 0 ? 'text-red-400' : remainingFunds === 0 ? 'text-warning' : 'text-primary'">
